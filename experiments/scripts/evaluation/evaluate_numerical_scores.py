...
def extract_scores_from_review(review_file: Path) -> List[Dict]:
    ...
    Supports both old format (version in filename) and new format (version in 'folder' field).
    ...
    try:
        ...
        # Determine version: new format has 'folder' field (e.g., 'v1', 'latest', 'authors_affiliation_good')
        # Old format has 'version' field or extracts from filename
        version = review_data.get('folder')  # New format
        if version is None:
            version = review_data.get('version')  # Old format
        if version is None:
            # Try to extract from filename: {version}_review_run{run_id}.json
            filename = review_file.name
            match = re.match(r'^([^_]+)_review_run', filename)
            if match:
                version = match.group(1)
            else:
                version = 'unknown'
        
        # Normalize version names (handle 'latest' vs folder structure)
        if version in ['latest', 'authors_affiliation_good', 'authors_affiliation_bad']:
            # For new folder structure, keep as is but map 'latest' to 'latest' for comparison
            pass
        elif version == 'v1':
            pass  # Keep as is
        else:
            # Try to map folder names to versions
            if 'v1' in version.lower():
                version = 'v1'
            elif 'latest' in version.lower() or version == 'latest':
                version = 'latest'
        ...
        else:  # default or unknown (includes new Gemini format)
...
def collect_all_scores(reviews_dir: Path) -> pd.DataFrame:
    ...
    - New: reviews_dir/{folder_name}/paper_id/review_run*.json
    ...
    # Check if this is new structure (has subdirectories like v1/, latest/, etc.)
    # or old structure (direct paper directories)
    subdirs = [d for d in reviews_dir.iterdir() if d.is_dir()]
    
    # Heuristic: if subdirectories contain paper-like folders (check first one)
    # New structure: reviews_dir/v1/paper_id/review_run*.json
    # Old structure: reviews_dir/paper_id/{version}_review_run*.json
    is_new_structure = False
    if subdirs:
        first_subdir = subdirs[0]
        # Check if it contains paper directories (has review_run*.json files inside subdirectories)
        paper_dirs_in_subdir = [d for d in first_subdir.iterdir() if d.is_dir()]
        if paper_dirs_in_subdir:
            # Check if any paper dir has review_run*.json
            for paper_dir in paper_dirs_in_subdir[:3]:  # Check first 3
                if list(paper_dir.glob("review_run*.json")):
                    is_new_structure = True
                    break
    
    if is_new_structure:
        # New structure: reviews_dir/{folder_name}/paper_id/review_run*.json
        print(f"Detected new folder structure")
        for folder_dir in subdirs:
            folder_name = folder_dir.name
            paper_dirs = [d for d in folder_dir.iterdir() if d.is_dir()]
        ...
        # Old structure: reviews_dir/paper_id/{version}_review_run*.json
        print(f"Detected old folder structure")
        paper_dirs = subdirs
